...
The Last Beacon ships in solo and two-player variants. Setup is a stepper — Concept, Adventure, Rules, Ready — that shows one step at a time with a progress indicator, so creating another draft replaces the current step view instead of appending a second form. Back and Next walk the steps that are open, in the order the chips show them, and no chip is closed while a later one is open (#259, #260). Below 700px the chips become numbered dots on one line under the “Step N of 4” line, so the indicator stays readable at 320px; each dot keeps its step name as its accessible label. Party assignment happens only once the service holds a draft, so it is the screen that follows the numbered steps rather than a step of them: creating the draft on Ready opens Party, and Party is offered below the chips for as long as a draft is open. The Adventure step asks one question — which adventure — and answers it with one control. Writing a scenario, starting from a bundled template and importing a scenario document are a library rather than a step of setting up a game, so they live under the **Scenarios** tab beside the game modes (#261), and the step links out to it. Choosing an authored adventure never silently rewrites an authored concept: a pristine setup is seeded from its first adventure, and after that the concept is kept and the adventure's is taken only through **Use adventure concept**, which **Restore previous concept** undoes (#258). Select a template, edit the brief and starting character purchases, review and create the draft, assign characters, mark ready and explicitly start. The server's existing compiler and atomic setup activation remain authoritative. Invitations bind to distinct authenticated player names; guests never receive the host's private scenario graph.
...
Setup carries the same landmarks the play shell does — a skip link, a header, a navigation region, one `main` and a footer — so the long, form-heavy flow can be navigated by landmark and the skip link has a real region to target (#257). The signed-in lobby leads with the games list it exists to show; signing out and refreshing that list are secondary controls beneath it, and no upkeep control is styled as a primary action (#204). The launcher's New game, Continue game and Join game controls are a tab bar: the selected mode is legible from the tab itself, and the panel it reveals is named by that tab rather than repeating the label as a heading. Setup and play are separate shells. Opening a campaign unmounts the launcher and the setup panel, so the game shell begins at the top of every route; the play header's **Session** menu returns to setup through **Switch campaign** (reopening the campaign that was being played) or **New game**, keeping the authenticated setup session in memory so no second sign-in is needed. **End session** lives in that menu with them. The header holds only two controls — the theme toggle and **Session** — grouped at its trailing edge; “Campaign companion” beside the logotype is a descriptor of the shell, set as serif italic prose so nothing in the header is styled as interactive without being interactive (#255).

Host lifecycle controls are separated from navigation and weighted by what they cost (#163). Ending a campaign is irreversible, so it asks first: a dialog names the campaign, says that a finished campaign can only be archived or continued and never returned to play, and defaults to **Keep playing**; only **End this campaign** sends the operation. Pausing is reversible, so it stays a single press and offers **Undo pause**, which resumes the campaign without entering play. **Open playing scene** and **Create another game** sit outside the lifecycle group.

The character editor separates what the rules fix from what a player chooses (#266). ST, DX, IQ and HT are four labelled steppers, always present in that canonical order: they carry no selector and no remove control, so an attribute cannot be deleted, bought twice or replaced by a skill, and a draft that reaches the editor missing or duplicating one is repaired before it can be edited. Add and remove belong to the sections that genuinely vary — advantages, disadvantages, skills, equipment, and the secondary characteristics a profile prices — and each of those selectors offers only definitions of its own section.
