...
Setup and play are separate shells. Opening a campaign unmounts the launcher and the setup panel, so the game shell begins at the top of every route; the play header's **Campaign** menu returns to setup through **Switch campaign** (reopening the campaign that was being played) or **New game**, keeping the authenticated setup session in memory so no second sign-in is needed.
