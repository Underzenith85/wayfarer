...
The Last Beacon ships in solo and two-player variants. Setup is a three-step wizard — Adventure, Rules, Ready — that shows one step at a time with a progress indicator, so creating another draft replaces the current step view instead of appending a second form. Back and Next walk the steps that are open, in the order the chips show them, and no chip is closed while a later one is open (#259, #260). Below 700px the chips become numbered dots on one line under the “Step N of 3” line, so the indicator stays readable at 320px; each dot keeps its step name as its accessible label. Party assignment happens only once the service holds a draft, so it is the screen that follows the numbered steps rather than a step of them: creating the draft on Ready opens Party, and Party is offered below the chips for as long as a draft is open. The Adventure step asks which published adventure to play. Writing a scenario, starting from a bundled template and importing a scenario document belong to a library rather than a game-setup step, so they live under **Create scenario** beside the other game modes (#261). Publish an authored scenario there, return to **Start game**, choose it under **Adventure and starting party**, confirm its pinned rules, review and create the draft, assign characters, mark ready and explicitly start. The server's compiler and atomic setup activation remain authoritative. Invitations bind to distinct authenticated player names; guests never receive the host's private scenario graph.
...
Setup carries the same landmarks the play shell does — a skip link, a header, a navigation region, one `main` and a footer — so the long, form-heavy flow can be navigated by landmark and the skip link has a real region to target (#257). Signing out and refreshing lists are secondary controls, and no upkeep control is styled as a primary action (#204). The launcher's **Start game**, **Join game** and **Create scenario** controls are a tab bar: the selected mode is legible from the tab itself, and the panel it reveals is named by that tab rather than repeating the label as a heading. Join game contains invitations, unfinished setups and authorized saved games. Setup and play are separate shells. Opening a campaign unmounts the launcher and setup panel, so the game shell begins at the top of every route; the play header's **Session** menu returns to setup through **Switch campaign** (opening Join game) or **New game** (opening Start game), keeping the authenticated setup session in memory so no second sign-in is needed. **End session** lives in that menu with them. The play header groups the theme toggle and **Session** at its trailing edge; its serif descriptor is the campaign name, falling back to “Field journal.” The setup masthead uses “Campaign companion” as its descriptor (#255).
