...
class RulesTests(unittest.TestCase):
    ...
    def test_attack_payloads_rejected(self):
        ...
        for c in cases:
            with self.subTest(character=c):self.assertFalse(builder.validate(c)['valid'])
...
class EngineTests(unittest.TestCase):
    def setUp(self):
        ...
        self.ai=patch('wayfarer.orchestration.llm.enabled',return_value=False); self.ai.start()
        self.s=engine.create(builder.character(),engine.scenario())
    def tearDown(self):
    ...
    def test_duplicate_no_reroll(self):
        with patch('wayfarer.rules.checks.roll',return_value={'success':False,'dice':[6,6,5]}) as dice:
    ...
    def test_invalid_character_cannot_activate(self):
        c=builder.character();c['attributes']['ST']=100
        with self.assertRaises(ValueError):engine.create(c,engine.scenario())
    def test_progression_and_reward_once(self):
        with patch('wayfarer.rules.checks.roll',return_value={'success':True,'dice':[1,1,1]}):
    ...
    def test_generation_failure_no_mutation(self):
        with patch('wayfarer.orchestration.llm.enabled',return_value=True),patch('wayfarer.orchestration.llm.generate',side_effect=ValueError('provider failed')):
    ...
    def test_narration_failure_keeps_commit(self):
        with patch('wayfarer.orchestration.llm.enabled',return_value=True),patch('wayfarer.orchestration.llm.generate',side_effect=[{'action':'rest'},ValueError('provider failed')]):
            s=self.turn('Rest')
