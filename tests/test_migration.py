...
class MigrationTests(unittest.TestCase):
    def test_original_schema_state_and_request_ids_survive(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            state = {'id':'legacy', 'revision':1, 'rules':'wayfarer-lite-1',
                     'character':builder.character(), 'scenario':scenario(),
                     'hp':10, 'fp':11, 'minutes':30, 'location':'Blackwater docks',
                     'inventory':['Travel clothes'], 'discoveries':[], 'flags':[],
                     'complete':False, 'messages':[{'role':'gm','text':'You rest.'}]}
            db = sqlite3.connect(path)
            with db:
                db.execute('CREATE TABLE campaigns (id TEXT PRIMARY KEY, state TEXT NOT NULL)')
                db.execute('CREATE TABLE events (campaign TEXT, request_id TEXT, payload TEXT, PRIMARY KEY(campaign, request_id))')
                db.execute('INSERT INTO campaigns VALUES (?, ?)', ('legacy', json.dumps(state)))
                db.execute('INSERT INTO events VALUES (?, ?, ?)', ('legacy','old-request',json.dumps({'input':'Rest'})))
            db.close()
            with patch.object(service, 'DB', path), patch('wayfarer.orchestration.llm.enabled', return_value=False):
