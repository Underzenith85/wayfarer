...
def test_selected_row_provenance_anchors() -> None:
    ...
    assert len(entries) == len(BASIC_EQUIPMENT.entries) + len(ULTRATECH_INDEX) == 23
    for entry in entries.values():
        ...
        assert set(provenance.pages) <= {271, 280, 283, 288}
...
def test_field_provenance_tracks_the_whole_equipment_schema() -> None:
    ...
    # are only exercised by synthetic combat fixtures.
...
def test_audit_report_names_blockers_without_claiming_completeness() -> None:
    ...
    assert report["selected_rows"] == 23
    assert report["supported_rows"] == 17
    assert report["sections_audited"] == 0
    assert isinstance(report["blockers"], list) and report["blockers"]
    assert report["footnotes_without_evidence"] == ["weapon-parry-modifier", "two-handed-weapon"]
