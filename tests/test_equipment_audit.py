...
def test_selected_row_provenance_anchors() -> None:
    ...
    assert len(entries) == len(BASIC_EQUIPMENT.entries) + len(ULTRATECH_INDEX) == 107
    for entry in entries.values():
        ...
        assert set(provenance.pages) <= {271, 272, 273, 274, 275, 276, 278, 280, 283, 287, 288}
...
def test_audit_report_names_blockers_without_claiming_completeness() -> None:
    ...
    assert report["selected_rows"] == 107
    assert report["supported_rows"] == 64
