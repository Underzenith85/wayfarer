...
RESIDUAL_OWNERS = (392, 393, 394, 395, 396, 397)
...
def test_no_mode_is_verified_and_each_one_says_why_under_a_live_owner() -> None:
    """#358 acceptance: verified with evidence, or carrying a concrete open child."""
    for mode, entry in MODES.items():
        assert not entry.verified, mode
        assert entry.residuals, mode
...
def test_the_report_publishes_every_residual_and_its_owner() -> None:
    ...
    assert report["verified_modes"] == 0
