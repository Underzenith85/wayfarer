...
def test_capability_inventory_is_stable_and_owned() -> None:
    ...
    assert all(
        entry.lite_required or entry.basic_required for entry in CAPABILITIES.values()
    )
    assert all(
        entry.basic_required for entry in CAPABILITIES.values() if entry.lite_required
    )
...
def test_conformance_fixture_contract_is_source_referenced_and_independent() -> None:
    ...
    assert by_id["lite-secondary-basic-speed"]["expected"] == {
        "basic_speed": "5.75"
    }
