...
GURPS_LITE = ProfileSelection(id=GURPS_LITE_PROFILE.id, version=1)
...
def test_default_registry_preserves_prototype_pins_and_rejects_gurps_until_verified() -> None:
    ...
    for profile in (GURPS_LITE_PROFILE, GURPS_BASIC_PROFILE):
        ...
        assert not any(d for p in profile.packages for d in p.definitions)
...
async def test_http_profile_listing_selection_and_migration(tmp_path: Path) -> None:
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        lite = listed[("profile:gurps-lite-4e-2004", 1)]
        ...
        response = await client.post(
            ...
            json={
                ...
                "brief": graph["brief"],
                "graph": graph,
                "rules_profile": {"id": "profile:gurps-lite-4e-2004", "version": 1},
            },
        )
