...
def test_selected_printing_ledgers_have_the_exhaustive_source_packet_denominator() -> None:
    ...
    assert not any(row.source_review == "reviewed" for row in bundle.rows)
    assert len(ledger_blockers(bundle.rows)) == len(bundle.rows)
...
def test_certification_reports_stable_ledger_blockers_and_rollups() -> None:
    ...
    assert report.source_ledger_rollups["source_review"] == {"pending": 1_285}
