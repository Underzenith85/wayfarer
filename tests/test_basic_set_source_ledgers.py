...
def test_selected_printing_ledgers_have_the_exhaustive_source_packet_denominator() -> None:
    ...
    assert sum(row.source_review == "reviewed" for row in bundle.rows) == 151
    # The reviewed animal-damage row remains partial for #522.
    assert len(ledger_blockers(bundle.rows)) == len(bundle.rows) - 150
