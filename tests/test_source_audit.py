...
def test_audit_integrity_and_unresolved_sources_block_certification() -> None:
    ...
    manifest = load(ROOT)
    assert sum(f.status == "compared" for f in manifest.fixtures) == 56
...
def test_mundane_skill_rows_carry_item_level_owners_and_certification_state() -> None:
    ...
    # #338-#341, #344 (with its children), #345, #346 and #356: a bound procedure
    # reaches certification as implemented, and a transferred one reaches it
    # naming the concrete open child that owns it.
    assert sum(r.implementation == "implemented" for r in rows) == 374
