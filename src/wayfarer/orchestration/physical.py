...
def _prepare(before: PlayState, command: PhysicalCommand, context: PhysicalContext) -> PreparedFeat:
    ...
    route = context.resolver(play, before, actor.actor_id, command.route_id)
    ...
    load = encumbrance(
        stats.profile_id,
        stats.basic_lift,
        Decimal(play.engine.resources.carried_weight(before.resources, actor.actor_id)) / 1000,
    )
    if load is None:
    ...
    state = injury_turn(play, before, actor.actor_id, command.id, start=True, do_nothing=False)
...
def _climb(feat: PreparedFeat, command: PhysicalCommand, rng: RandomSource) -> FeatOutcome:
    ...
    check = _roll(
        ...
        command,
        rng,
        _skill(feat.compiled, "climbing", stats.dx - 5)
        + modifier
        - int(load)
    ...
    if not check.outcome.succeeded:
        ...
        damage = max(0, sum(rng.randbelow(6) + 1 for _ in range(dice)) + adds)
...
def reduce_physical(
    ...
    outcome = (
        _FEATS[command.kind](feat, command, context.play.rng) if feat.allowed else FeatOutcome()
    )
...
class PhysicalService:
    ...
    async def execute(
        ...
        context = PhysicalContext(play, self.resolver, self.play.rng)
