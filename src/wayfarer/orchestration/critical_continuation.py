...
def context(
    play: PlayService, state: PlayState, encounter: Encounter, critical_id: str
) -> CriticalMiss:
    ...
    if build(play, state, saved.subject_id).revision != saved.build_revision:
        raise ConflictError("Original critical build is no longer active")
    if hashlib.sha256(catalog(play).model_dump_json().encode()).hexdigest() != saved.catalog_digest:
...
def continue_critical(
    ...
    *,
    ...
    saved = context(play, state, encounter, critical_id)
    ...
    if stage == "migrate":
        ...
        if saved.table_total == 15:
            ...
            die = play.rng.randbelow(6) + 1 if len(hands) > 1 else None
            ...
            body, side = (play.rng.randbelow(6) + 1 for _ in range(2))
            location = (
                ("right-arm" if side <= 3 else "left-arm")
                if body <= 3
                else ("right-leg" if side <= 3 else "left-leg")
            )
            damage_dice = tuple(play.rng.randbelow(6) + 1 for _ in range(weapon.dice))
            ...
            stats = build(play, state, saved.subject_id).statistics
        ...
        if saved.incoming:
            ...
            incoming_dice = tuple(play.rng.randbelow(6) + 1 for _ in range(incoming.dice))
            dice += incoming_dice
            resources, wound = apply_injury(
                resources,
                Wound(
