...
def checkpoint(play: PlayService, state: PlayState, before: PlayState) -> PlayState:
    ...
    for effect in active_spells(resources):
        ...
        if lost:
            damage = sum(play.rng.randbelow(6) + 1 for _ in range(effect.energy))
            resources, _ = apply_injury(
                ...
                Wound(
                    ...
                    basic_damage=damage,
                    resistance=armor(play, state, effect.actor_id),
                    damage_type="burn",
                ),
...
def concentration_checkpoint(play: PlayService, state: PlayState, before: PlayState) -> PlayState:
