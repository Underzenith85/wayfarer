...
from wayfarer.simulation.combat import Defense, Encounter, InjuryTrace


def combat_value(play: PlayService, state: PlayState, actor_id: str) -> DerivedValue:
    ...
    return EffectEvaluator((MechanicalTarget("attribute:dx"),)).evaluate(
        "attribute:dx",
        base,
        play.engine.resources.equipment_effects(state.resources, actor_id),
...
def resolve_injury(
    play: PlayService, state: PlayState, encounter: Encounter, selected: Defense
    ...
    if hit and attack.outcome != Outcome.CRITICAL_SUCCESS and selected != "none":
        defense_value = combat_value(play, state, pending.defender_id)
    ...
    dice = tuple(play.rng.randbelow(6) + 1 for _ in range(profile.damage_dice)) if hit else ()
