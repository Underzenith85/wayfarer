...
def resolve_flight(
    play: PlayService, state: PlayState, encounter: Encounter, table: tuple[int, ...]
) -> tuple[PlayState, Encounter, tuple[int, ...]]:
    from wayfarer.orchestration.gurps_melee import build, catalog, mode
    ...
    distance, direction = (play.rng.randbelow(6) + 1 for _ in range(2))
    ...
    compiled = build(play, state, subject.actor_id)
    ...
    for target in encounter.participants:
        ...
        if not check.outcome.succeeded:
            ...
            if play.engine.rules.abilities:
