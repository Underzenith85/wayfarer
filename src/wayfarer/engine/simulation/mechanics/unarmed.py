...
from wayfarer.engine.rules.combat_tables import strong_damage_bonus
from wayfarer.engine.rules.gurps_checks import success_roll
from wayfarer.engine.rules.location_types import Hand, HumanLocation
...
from wayfarer.engine.simulation.maneuvers import ManeuverState, WaitInterrupt, WaitTrigger
...
from wayfarer.engine.simulation.mechanics.location_combat import disabled, unavailable_hand
from wayfarer.engine.simulation.physical_traits import physical_traits
from wayfarer.engine.simulation.unarmed import (
    BASIC,
    GrappleLocation,
    Grip,
    PendingUnarmed,
...
def validate_action(
    ...
    from wayfarer.engine.simulation.hit_locations import require_location
    ...
    from wayfarer.engine.simulation.tactical import attack_geometry
    ...
    if command.action in ("punch", "kick", "grapple", "arm_lock"):
        ...
        if command.enter_close_combat and encounter.spatial_kind == "hex":
            ...
            from wayfarer.engine.simulation.tactical import occupants, pose
...
def execute_unarmed(
    ...
    from wayfarer.engine.simulation.combat_commands import ChooseDefense
...
def unarmed_defense(
    ...
    height_bonus = 0
    if encounter.spatial_kind == "hex":
        from wayfarer.engine.simulation.tactical import defense_adjustment, height_effect
...
def encumbrance_level(runtime: RulesContext, state: PlayState, actor_id: str) -> int:
    from wayfarer.engine.simulation.gurps_equipment import inventory_load
...
def defend(
    ...
    from wayfarer.engine.simulation.maneuvers import attack_modifier
...
def armor_dr(
    ...
    *,
    ...
    from wayfarer.engine.simulation.hit_locations import armor_resistance
...
def critical_miss(
    ...
    if hand not in (None, "left-hand", "right-hand"):
        from wayfarer.engine.simulation.combat import PendingDefense
    ...
    if rule.effect in ("strain", "self-hit"):
        from wayfarer.engine.simulation.gurps_equipment import MeleeMode
        from wayfarer.engine.simulation.injury import DisableLocation, apply_location_effect
