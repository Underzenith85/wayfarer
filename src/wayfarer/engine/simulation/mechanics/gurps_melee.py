...
from wayfarer.engine.rules.combat_tables import minimum_strength_penalty, strong_damage_bonus
...
from wayfarer.engine.rules.mundane_skills.ranged import require_technology
...
from wayfarer.engine.simulation.entangle import defense_penalty as entangle_defense_penalty
from wayfarer.engine.simulation.entangle import immobilized
from wayfarer.engine.simulation.fatigue import ContinueExertion, apply_fatigue, fatigue_value
...
from wayfarer.engine.simulation.maneuvers import ATTACK_MANEUVERS, attack_modifier
...
def injury_turn(
    ...
    *,
    ...
    from wayfarer.engine.simulation.spell_backfires import clear_stun, mental_stun, refund_due
...
def mode(
    ...
    from wayfarer.engine.simulation.mechanics.location_combat import (
        disabled,
        item_hands,
        unavailable_hand,
    )
...
def defense_height_bonus(
    ...
    from wayfarer.engine.simulation.tactical import pose
...
def defense_value(
    ...
    *,
    ...
    from wayfarer.engine.simulation.fright import can_defend
    ...
    from wayfarer.engine.simulation.mechanics.location_combat import disabled, item_hands
    ...
    if targeted_weapon:
        bonus = 0
    from wayfarer.engine.simulation.fright import stunned as fright_stunned
            ...
            for weapon_mode in entry.modes:
                ...
                if incoming_weight is not None:
                    ...
                    if 0 < 3 * entry.weight_millipounds <= incoming_weight:
                        from wayfarer.engine.simulation.mechanics.heavy_parry import (
                            require_breakage,
                        )
...
def prepare_attack(
    ...
    *,
    ...
    if isinstance(selected, RangedMode):
        from wayfarer.engine.simulation.mechanics.gurps_ranged import prepare
    ...
    from wayfarer.engine.simulation.combat import BasicSpatialContext, CombatEngine, basic_distance
    from wayfarer.engine.simulation.tactical import attack_geometry, defense_adjustment
...
def resolve_melee(
    ...
    *,
    ...
    assert pending is not None
    if pending.spell_cast_id is not None:
        from wayfarer.engine.simulation.mechanics.spell_missiles import resolve as resolve_spell
    ...
    from wayfarer.engine.simulation.mechanics.object_combat import shock
    ...
    if hit and attack.outcome is not Outcome.CRITICAL_SUCCESS and defense_derived is not None:
        ...
        if (
            ...
            from wayfarer.engine.simulation.mechanics.heavy_parry import resolve_heavy_parry
    ...
    if blocked and blocked.startswith("basic-critical-miss:"):
        from wayfarer.engine.simulation.mechanics.critical_limbs import resolve_limb
    ...
    if blocked and blocked.startswith("basic-critical-miss:"):
        from wayfarer.engine.simulation.mechanics.object_combat import critical_breakage
    ...
    from wayfarer.engine.simulation.mechanics.object_combat import shield_damage
    ...
    if hit and pending.target_item_id:
        from wayfarer.engine.simulation.mechanics.object_combat import synchronize
    ...
    if blocked and sum(critical_dice) == 14 and not blocked.endswith(":defender"):
        from wayfarer.engine.simulation.mechanics.weapon_flight import resolve_flight
    ...
    if blocked and blocked.startswith("basic-critical-miss:"):
        from wayfarer.engine.simulation.critical import IncomingWound
        from wayfarer.engine.simulation.mechanics.critical_context import capture_critical
        from wayfarer.engine.simulation.mechanics.location_combat import from_behind
    ...
    from wayfarer.engine.simulation.mechanics.location_combat import unavailable_hand
