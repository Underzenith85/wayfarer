...
from wayfarer.engine.simulation.gurps_equipment import MeleeMode, RangedMode, WeaponMode
...
def resolve_limb(
    ...
    *,
    ...
    number = sum(table)
    if number == 15:
        die = draw_dice(runtime.rng, 1)[0] if len(hands) == 2 else None
        ...
        mode = modes[0]
        body_die, side_die = (draw_dice(runtime.rng, 1)[0] for _ in range(2))
