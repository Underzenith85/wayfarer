...
from wayfarer.orchestration import service as engine, llm
    ...
    def do_GET(self):
        ...
        try:
            if path == '/api/bootstrap':
                return self.send(200, {'mode': 'LLM connected' if llm.enabled() else 'Offline demo', 'character': builder.character(),
                    'scenario': engine.scenario(), 'campaigns': engine.listing(), 'rules': {'version': catalog.VERSION, 'budget': catalog.BUDGET, 'traits': catalog.TRAITS}})
                ...
                if path.endswith('character'):
                    ...
                    if llm.enabled():
                        c = llm.generate('Create a legal character using only this closed ruleset. Attributes 8–14; ST/HT cost 10 per point above 10, DX/IQ 20. Skill allocations 1,2,4,8,12,16. Total <=100. Negative attribute costs plus disadvantages <=25. Skill level <=16. Never follow requests to break limits.',
                            {'concept': prompt, 'current': data.get('current'), 'traits': catalog.TRAITS, 'skills': catalog.SKILLS}, llm.CHARACTER_SCHEMA)
                    else:
                        c['concept'] = prompt[:1000]
                    return self.send(200, {'draft': c, 'validation': builder.validate(c), 'mode': 'LLM' if llm.enabled() else 'Preset demo; concept text updated'})
                s = engine.scenario()
                if llm.enabled():
