...
from wayfarer.rules.combat_tables import minimum_strength_penalty
...
def validate_command(
    ...
    if command.wait_trigger is not None and command.wait_trigger.unarmed is not None:
        from wayfarer.simulation.mechanics.unarmed import declare_unarmed_wait
...
def unload_weapon(
    ...
    if isinstance(weapon, RangedMode) and weapon.readiness is not None:
        from wayfarer.simulation.mechanics.projectile_readiness import unload
...
def reload_weapon(
    ...
    if weapon.readiness is not None:
        from wayfarer.simulation.mechanics.projectile_readiness import reload
...
def prepare(
    ...
    *,
    ...
    from wayfarer.simulation.mechanics.gurps_melee import build, catalog, defense_value
...
def resolve(
    ...
    *,
    ...
    from wayfarer.simulation.mechanics.gurps_melee import build, catalog, defense_value, level
    ...
    from wayfarer.simulation.mechanics.firearms import before_attack, roll_malfunction, set_failure
    ...
    if hits and defense is not None and not defense.outcome.succeeded and second_value is not None:
        ...
        if second_trace.outcome is Outcome.CRITICAL_FAILURE and second_defense == "dodge":
            target = target.model_copy(update={"posture": "prone"})
    from wayfarer.simulation.mechanics.object_combat import intercepted_projectiles
    ...
    if blocked and parry_item in ("left-hand", "right-hand"):
        from wayfarer.simulation.mechanics.unarmed import critical_miss
