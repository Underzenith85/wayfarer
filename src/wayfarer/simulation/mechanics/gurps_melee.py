...
from wayfarer.rules.checks import Outcome, draw_dice
from wayfarer.rules.combat_tables import minimum_strength_penalty, strong_damage_bonus
...
def mode(
    ...
    from wayfarer.simulation.mechanics.location_combat import disabled, item_hands, unavailable_hand
...
def defense_value(
    ...
    *,
    ...
    from wayfarer.simulation.mechanics.object_combat import weapon_target
    ...
    from wayfarer.simulation.mechanics.location_combat import disabled, item_hands
            ...
            for weapon_mode in entry.modes:
                ...
                if incoming_weight is not None:
                    ...
                    if 0 < 3 * entry.weight_millipounds <= incoming_weight:
                        from wayfarer.simulation.mechanics.heavy_parry import require_breakage
...
def prepare_attack(
    ...
    *,
    ...
    if isinstance(selected, RangedMode):
        from wayfarer.simulation.mechanics.gurps_ranged import prepare
    ...
    geometry = encounter
    if target_item_id:
        from wayfarer.simulation.mechanics.object_combat import target_geometry
...
def resolve_melee(
    ...
    *,
    ...
    assert pending is not None
    if pending.spell_cast_id is not None:
        from wayfarer.simulation.mechanics.spell_missiles import resolve as resolve_spell
    ...
    from wayfarer.simulation.mechanics.object_combat import shock
    ...
    from wayfarer.simulation.mechanics.object_combat import intercepting_shield
    ...
    if hit and attack.outcome is not Outcome.CRITICAL_SUCCESS and defense_derived is not None:
        ...
        if (
            ...
            from wayfarer.simulation.mechanics.heavy_parry import resolve_heavy_parry
    ...
    if blocked and blocked.startswith("basic-critical-miss:"):
        from wayfarer.simulation.mechanics.critical_limbs import resolve_limb
    ...
    if blocked and blocked.startswith("basic-critical-miss:"):
        from wayfarer.simulation.mechanics.object_combat import critical_breakage
    ...
    if blocked and sum(critical_dice) == 14 and not blocked.endswith(":defender"):
        from wayfarer.simulation.mechanics.weapon_flight import resolve_flight
    ...
    if blocked and blocked.startswith("basic-critical-miss:"):
        ...
        from wayfarer.simulation.mechanics.critical_context import capture_critical
        from wayfarer.simulation.mechanics.location_combat import from_behind
    ...
    from wayfarer.simulation.mechanics.gurps_maneuvers import distracted
    ...
    from wayfarer.simulation.mechanics.location_combat import unavailable_hand
